...
admin_ingressgateway_ip=$(cd "${parent_path}/../.." && make -s +on-prem-lite-get-admin-ingressgateway-ip)
keycloak_ingressgateway_ip=$(cd "${parent_path}/../.." && make -s +on-prem-lite-get-keycloak-ingressgateway-ip)
tenant_ingressgateway_ip=$(cd "${parent_path}/../.." && make -s +on-prem-lite-get-tenant-ingressgateway-ip)
